...
test:
	pytest --cov=ozon_api --cov-report=xml --cov-report=html --cov-report=term-missing

test-quick:
	pytest --tb=short

lint:
	python -m black --check ozon_api tests

type-check:
	python -m mypy ozon_api

format:
	black ozon_api/ tests/
	isort ozon_api/ tests/
